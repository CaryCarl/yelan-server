...
router.post("/uploadCOS", upload.single('file'), async (req, res) => {
	...
	try {
		...
		const result = await cos.putObject({
			Bucket: 'juzi-1253562145', // 格式：<BucketName>-<APPID>
			Key: ossFilePath,
			Region: 'ap-guangzhou',
...
function getCurrentTimeString() {
	...
	const month = String(now.getMonth() + 1).padStart(2, '0'); // 月（补零）
	const day = String(now.getDate()).padStart(2, '0'); // 日（补零）
	const hours = String(now.getHours()).padStart(2, '0'); // 时（补零）
	const minutes = String(now.getMinutes()).padStart(2, '0'); // 分（补零）
